...
Public Class FrmEscaneoPagos
    ...
    Dim _ruta As String = ConfigDiv.xDameValor(E_ConfiguracionesDiversas.eClaves.RUTA_BUZON_ENTRADA_PAGOS)
                ...
                For indice As Integer = 0 To GridView1.RowCount - 1
                    ...
                    If Not IsNothing(row) Then
                        ...
                        If row("S") = True Then
                            ...
                            If nombre_archivo.ToLower.StartsWith("auto-pp") Then
    ...
    Private Sub ActualizarDatosPagoAsiento(row As DataRow, ByRef clave As String, ByRef documento As String)
        ...
        Dim nombre_archivo As String = nombre.ToLower.Replace("auto-", "").Replace("pp", "").Replace(".pdf", "")
        Dim Datos_guion As String() = Split(nombre_archivo, "-")
        Dim datos_guion_bajo As String() = Split(Datos_guion(0).ToString, "_")
        Dim datos_pago As String() = Split(datos_guion_bajo(0).ToString, ".")


        Dim IdEmpresa As String = "" : If datos_pago.Length > 0 Then IdEmpresa = VaInt(datos_pago(0)).ToString
        Dim IdPago As String = "" : If datos_pago.Length > 1 Then IdPago = VaInt(datos_pago(1)).ToString
        ...
        If VaDec(IdPago) > 0 Then

            Dim IdAsiento As String = ""


            'Obtenemos el asiento del pago
            'Actualizamos el documento y el banco del pago 
            Dim Pagos As New E_Pagos(Idusuario, ConexCtb(VaInt(IdEmpresa)))
            If Pagos.LeerId(IdPago) Then

                IdAsiento = (Pagos.IdAsiento.Valor & "").Trim




                Pagos.Numero.Valor = documento_pago
                Pagos.IdBanco.Valor = IdBanco
                Pagos.Actualizar()
