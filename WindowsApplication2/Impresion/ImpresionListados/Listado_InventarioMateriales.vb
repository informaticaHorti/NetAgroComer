Public Class Listado_InventarioMateriales
    ...
    Private Sub ImprimeCabecera()
        ...
        If Marcas Then
            Texto = " Cod | Material | Marca | ExistIni | ValIni | UdsCom | ValCom | MovEnv | ExPalets | "
            Texto = Texto & "Exist | Recuento | Difer | P.M.C. | ValExist | ValRec"
            Formato = "13|35|15|>15|>17|>17|>17|>15|>17|>18|>15|>18|>18|>18|>18|>18"
        Else
            Texto = "Cod | Material | ExistIni | ValIni | UdsCom | ValCom | MovEnv | ExPalets | "
            Texto = Texto & "Exist | Recuento | Difer | P.M.C. | ValExist | ValRec"
    ...
    Private Sub ImprimeDetalle(ByRef totalFamilia As AcumuladorTotales)
        ...
        For Each r As DataRow In Dt.Rows
            ...
            Dim existIni As Decimal = VaDec(r("ExiIni"))
            Dim valIni As Decimal = VaDec(r("VExiIni"))
            Dim udsCom As Decimal = VaDec(r("UdsCom"))
            Dim valCom As Decimal = VaDec(r("VCom"))
            Dim movEnv As Decimal = VaDec(r("MovEnv"))
            Dim exPalets As Decimal = VaDec(r("ExiPal"))
        ...
        Dim TotalExistIni As Decimal = VaDec(Dt.Compute("SUM(ExiIni)", ""))
        Dim TotalValIni As Decimal = VaDec(Dt.Compute("SUM(VExiIni)", ""))
        Dim TotalUdsCom As Decimal = VaDec(Dt.Compute("SUM(UdsCom)", ""))
        Dim TotalValCom As Decimal = VaDec(Dt.Compute("SUM(VCom)", ""))
        Dim TotalMovEnv As Decimal = VaDec(Dt.Compute("SUM(MovEnv)", ""))
        Dim TotalExPalets As Decimal = VaDec(Dt.Compute("SUM(ExiPal)", ""))
